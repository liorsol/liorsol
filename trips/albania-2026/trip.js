...
/* ============================================================
   Weather — Open-Meteo forecast, no key, CORS-open, one request for all four
   places. Each trip day is shown for the place we sleep that night, because
   the villa sits ~850 m up and runs 6–8° cooler than Tirana; `elevation`
   tells the API to downscale instead of handing back the valley's numbers.
   ...
   back to Tirana, which is also the default the family asked for. Any failure
   leaves the card hidden — a trip page must never show a stale temperature,
   which is also why the service worker never caches this request.
   ============================================================ */
(function(){
  ...
  var SPOTS = {
    tirana: {n:'טירנה',     lat:41.3275, lng:19.8187, el:110},
    rinas:  {n:'אזור השדה', lat:41.4192, lng:19.7142, el:30},
    villa:  {n:'שן-ג׳רג׳',  lat:41.3376, lng:20.0955, el:850},
    ksamil: {n:'קסאמיל',    lat:39.7647, lng:19.9967, el:10}
  };
  /* where we are meant to be, night by night — mirrors the agenda view */
  var PLAN = {
    '2026-08-13':'rinas',  '2026-08-14':'villa',  '2026-08-15':'villa',
    '2026-08-16':'villa',  '2026-08-17':'villa',  '2026-08-18':'villa',
    '2026-08-19':'ksamil', '2026-08-20':'ksamil', '2026-08-21':'ksamil',
    '2026-08-22':'rinas'
  };
  ...
  var url = 'https://api.open-meteo.com/v1/forecast?latitude=' + field('lat') +
    ...
    '&timezone=Europe%2FTirane&forecast_days=16';
  ...
  fetch(url).then(function(r){
    ...
    if(!daily.tirana || !daily.tirana.time.length) return;

    var today = daily.tirana.time[0], days = [], sub;
    ...
    } else {                         // trip out of range → the default, Tirana
      var d = daily.tirana;
      for(var i = 0; i < Math.min(5, d.time.length); i++){
        days.push({date:d.time[i], spot:SPOTS.tirana.n, i:d.weather_code[i],
                   hi:d.temperature_2m_max[i], lo:d.temperature_2m_min[i]});
      }
      sub = 'הטיול עדיין רחוק מכדי תחזית — בינתיים מזג האוויר בטירנה.';
    }
    ...
    days.forEach(function(x){
      ...
      [['d', parts[2] + '.' + Number(parts[1])], ['i', icon(x.i)],
       ['t', round(x.hi) + ' / ' + round(x.lo)], ['w', x.spot]].forEach(function(pair){
...
/* ============================================================
   ...
   one EUR-based response gives every pair the family actually needs.
   Indicative only — an ATM or a bureau will not match it to the lek.
   ============================================================ */
(function(){
  ...
  fetch('https://open.er-api.com/v6/latest/EUR').then(function(r){
    ...
    var all = d && d.rates && d.rates.ALL, ils = d && d.rates && d.rates.ILS;
    if(!all || !ils) return;
    [['€1', all.toFixed(0) + ' לק'],
     ['₪1', (all / ils).toFixed(0) + ' לק'],
     ['1,000 לק', '₪' + (1000 * ils / all).toFixed(0)],
     ['€1', '₪' + ils.toFixed(2)]].forEach(function(pair){
      var s = document.createElement('span');
      s.textContent = pair[0] + ' ≈ ' + pair[1];
      box.appendChild(s);
    });
    var src = document.getElementById('fx-src');
    src.textContent = 'שער יציג לפי open.er-api.com, עודכן ' +
      new Date(d.time_last_update_unix * 1000).toLocaleDateString('he-IL') +
      ' · אינדיקטיבי בלבד — בכספומט ובחלפנים השער יהיה פחות טוב.';
...
/* ============================================================
   ...
   ============================================================ */
(function(){
  var CITY = {tirana:'טירנה', berat:'בראט', saranda:'סרנדה', ksamil:'קסאמיל',
              gjirokaster:'ג׳ירוקסטרה', durres:'דורס', shkoder:'שקודרה'};
  ...
  fetch('restaurants.json', {cache:'no-cache'}).then(function(res){ return res.json(); }).then(function(list){
    ...
    box.appendChild(link('https://github.com/liorsol/liorsol/blob/main/trips/albania-2026/restaurants.json',
                         null, 'הרשימה המלאה ↖'));
  });
})();
...
/* ============================================================
   ...
   *all* of them, in one chronological thread. A note left on `north` used to
   ...
   ============================================================ */
(function(){
  var DB   = 'https://liorsol-github-default-rtdb.europe-west1.firebasedatabase.app/albania2026';
  var IDRE = /^[a-z0-9]{1,10}_[a-z0-9]{4}$/;    // must match the DB rules
  var LIM  = {name:24, text:800, title:100, url:500};
  var CACHE = 'albania2026_cache';              // last good read, for offline
  var QUEUE = 'albania2026_queue';              // writes waiting for a network
